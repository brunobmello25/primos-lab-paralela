...
int main(int argc, char *argv[]) {
  ...
  // Caso o número de processos seja maior que 1, então será necessário que os
  // processos enviem os resultados para o 0.
  if (num_procs > 1) {
    ...
    if (meu_ranque != 0) {
      ...
      for (int source = 1; source < num_procs; source++) {
        ...
        MPI_Request request;
        MPI_Irecv(&received_cont, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD,
                  &request);
